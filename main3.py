...
def get_source_info(author_name):
    search_query = scholarly.search_author(author_name)
    author = scholarly.fill(next(search_query))
    author_id = author['scholar_id']
    params = {
    ...
    for source in results:
        source_info_array.append([source, source_info[source]])

    return source_info_array
...
def create_graphic():
    ...
    headings_source = ['Source', 'Quantity']
    publication_info = []
    publisher_info = []
    source_info = []
    layout = [
        ...
        [sg.Text('Define Paper number'), sg.Input(key='-AUTHOR-')],
        ...
        [sg.Table(values=publication_info, headings=headings_publication, max_col_width=50,
                  ...
                  row_height=15)],
        [sg.Table(values=source_info, headings=headings_source, max_col_width=50,
                  auto_size_columns=True,
                  display_row_numbers=True,
                  justification='right',
                  num_rows=20,
    ...
    window = sg.Window('GUI for Google Scholar Queries', layout, size=(950, 900))
